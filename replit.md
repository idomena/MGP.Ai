...
- **Key Features**: Conversational onboarding chat (11-step flow with interactive SVG body diagram for muscle focus, personalized coach name, progress bar), iPhone-optimized workout page with add exercise functionality and smart muscle group suggestions, muscle anatomy diagrams, workout customization (change type, swap exercises, add exercises), AI scheduling assistant, redesigned journey path visualization, real-time progress updates, enhanced workout session UI with rest timers and completion animations, in-app + browser notification system, persistent AI chat with conversation history, exercise preview details in add/swap exercise modals, exercise reordering (drag & drop + up/down buttons), exercise deletion for ALL exercises with confirmation dialog, and Screen Wake Lock API to prevent screen timeout during workouts.
- **Exercise Filtering**: AddExerciseModal uses a two-step flow: (1) Interactive body diagram for muscle group selection, (2) Filtered exercises from Supabase `exercises_library` table with local data fallback. SwapExerciseModal uses `getAlternativesForMuscleGroup()` for same-muscle-group alternatives. Color-coded muscle group badges (chest=red, back=blue, shoulders=orange, arms=purple, legs=green, core=pink) in both modals and workout exercise cards. GIF thumbnails on workout exercise cards.
- **Supabase exercises_library Integration**: `src/services/exercisesLibraryService.ts` fetches exercises from the `exercises_library` table, maps them to the app's Exercise interface with stable hash-based IDs. Falls back to local exercise data when Supabase is not configured. `useExercises` hook (`src/hooks/useExercises.ts`) also fetches from Supabase with local fallback.
- **Interactive Body Diagram**: `src/components/InteractiveBodyDiagram.tsx` - clickable SVG front/back body views with 6 selectable muscle groups. Color-coded visual states (default/highlighted/selected with glow animation). Accessible with role=button, tabIndex, aria-labels, keyboard support.
- **Exercise Persistence**: Added exercises (via modal or AI assistant) are persisted to localStorage per user+day (`added_exercises_${userId}_day${dayNumber}`). Hidden/deleted exercises stored in `hidden_exercises_${userId}_day${dayNumber}`. Survives page refresh. Uses ref-based guard to prevent overwriting saved data on initial render. Custom exercise order persisted to localStorage (`exercise_order_${userId}_day${dayNumber}`).
- **Screen Wake Lock**: WorkoutSession component uses Screen Wake Lock API to prevent screen timeout during active workouts. Auto-releases on workout exit/completion. Re-acquires on visibility change (app switch).
- **AI Chat**: Conversations saved to localStorage per user (max 100 messages). Last 20 messages sent as context to Gemini API for multi-turn conversations. "New Chat" button to start fresh. System prompt uses friendly, supportive tone with short sentences and check-in questions.
